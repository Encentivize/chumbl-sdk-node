...
if (process.env.CHUMBL_BASE_URL) {
    ...
    if (_environment === "development") {
        ...
        _baseUrl = "http://chumbl-api-qa.encentivize.co.za";
    } else {
...
function retryableCallApi(requestOptions, options, callback) {
    ...
    try {
        request[options.verb](requestOptions, function (error, response, body) {
            ...
            if (alwaysRetryableStatusCodes.indexOf(response.statusCode) >= 0) {
...
function apiCallComplete(requestOptions, options, callback, error, result) {
    if (error) {
        return callback("Error with request object : " + options.verb + " : " + requestOptions.url + " : " + JSON.stringify(error));
    }
    if (result.error) {
        return callback("Error connecting to api : " + options.verb + " : " + requestOptions.url + " : " + JSON.stringify(result.error));
    }
